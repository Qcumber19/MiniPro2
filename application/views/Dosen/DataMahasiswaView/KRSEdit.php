...
<head>
	...
	<title>Dashboard - KRS(Edit)</title>
	<meta content='width=device-width, initial-scale=1.0, shrink-to-fit=no' name='viewport' />
	<link rel="icon" href="<?php echo base_url('/assets/img/icon.ico" type="image/x-icon')?>"/>

	<style>
		thead, tbody, tr, td, tfoot, select{
			color: white;
		}
		option{
			color: #373b38;
...
<body data-background-color="dark">
						...
						<div class="col-md-12">
							<div class="card">
								<div class="card-header">
									<div class="card-title">KARTU RENCANA STUDI (EDIT)</div>
								</div>
  <?php
  ...
  <?php } ?>
								...
								<div class="card-body">
									<?php echo form_open('admin/krs_edit'); ?>
									<input type="hidden" name="id_krs" value="<?= $krs['id_krs']; ?>">
											<div class="form-group form-floating-label <?php echo(form_error('mahasiswa') ? 'has-error' : '') ?>">
            									...
            									<option value="<?= $show->nama; ?>" <?= ($krs['nama'] == $show->nama ? 'selected' : '' ); ?>>
                								(<?= $show->nim ?>) 
                								<?= $show->nama; ?>
            									</option>
        										<?php endforeach ?>
											...
											<div class="form-group <?php echo (form_error('jurusan') ? 'has-error' : '') ?>">
        										...
        										<select style="color: white" class="form-control input-border-bottom" id="selectFloatingLabel" required name="jurusan">
                									...
                									<option value="Sistem Informasi" <?= ($krs['jurusan'] == 'Sistem Informasi' ? 'selected' : '' ); ?>>Sistem Informasi</option>
        										</select>
                        ...
                        <?php foreach ($d as $d): ?>
                            <span style="" class="btn btn-default"><?= $d ?></span>
                        <?php endforeach ?>
									...
									<table class="table table-head-bg-primary mt-4">
										...
										<tbody>
											<?php $this->load->model('M_Dosen'); ?>
                    						<?php $matakuliah = $this->M_Dosen->nama($x->semester); ?>
